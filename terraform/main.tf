...
resource "aws_s3_bucket" "website_bucket" {
  bucket = "${var.domain_name}-${random_string.suffix.result}"
}
...
# CloudFront Distribution pointing to the S3 bucket website endpoint
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  price_class = "PriceClass_100" # Cheapest price class, mostly US/EU
